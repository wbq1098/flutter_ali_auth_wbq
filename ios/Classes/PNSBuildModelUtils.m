...
+ (TXCustomModel *)buildFullScreenModel {
  ...
  model.navColor = [self colorWithHex:0xFFA346 alpha: 1]; //UIColor[0xFF3A71FF];
    model.navTitle = [[NSAttributedString alloc] initWithString:@"本机号码一键登录" attributes:@{NSForegroundColorAttributeName : UIColor.whiteColor,NSFontAttributeName : [UIFont systemFontOfSize:20.0]}];
    ...
    model.privacyColors = @[[self colorWithHex:0x9B9B9B alpha: 1], [self colorWithHex:0x2D8AD1 alpha: 1]];
    ...
    model.changeBtnTitle = [
       ...
       attributes:@{NSForegroundColorAttributeName: [self colorWithHex:0x9B9B9B alpha: 1], NSFontAttributeName : [UIFont systemFontOfSize:18.0]}
    ];
...
+ (TXCustomModel *)buildAlertModel {
    ...
    model.alertTitleBarColor = [self colorWithHex:0xFFA346 alpha: 1];
